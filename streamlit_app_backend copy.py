...
with st.sidebar:
    ...
    with col_inp1:
        if st.button(
            ...
            key="inp_lang_en",
            use_container_width=True,
            type="primary" if st.session_state.input_language == "English" else "secondary"
        ):
            st.session_state.input_language = "English"
            st.rerun()
    with col_inp2:
        if st.button(
            ...
            key="inp_lang_hi",
            use_container_width=True,
            type="primary" if st.session_state.input_language == "Hindi" else "secondary"
        ):
            st.session_state.input_language = "Hindi"
            st.rerun()

    st.caption(f"📍 **{st.session_state.input_language}**")
...
if st.session_state.active_chat_id:
                ...
                if sender == "user":
                    ...
                    if is_img_analysis:
                        ...
                        with st.chat_message("assistant"):
                            ...
                            if ref_imgs:
                                with st.expander(f"🖼️ Similar Reference Images ({len(ref_imgs)} found)", expanded=False):
                                    for ri in ref_imgs:
                                        ...
                                        if os.path.exists(img_path):
                                            ...
                                            st.caption(f"📄 {ri.get('disease', 'Unknown')} (similarity: {ri.get('similarity_score', 0):.2f})")
    ...
    with col1:
        _chat_placeholder = (
            "आलू रोगों, उर्वरकों, खेती के बारे में पूछें..."
            if st.session_state.input_language == "Hindi"
            else "Ask about potato diseases, fertilizers, cultivation..."
        )
    ...
    if user_query:
        # If the user is typing in Hindi, append a language hint so the LLM
        # can correctly interpret the Hindi question against English-indexed context.
        if st.session_state.input_language == "Hindi":
            user_query_for_backend = user_query + "\n\n[Note: The above question is written in Hindi. Please understand the Hindi question and answer accordingly.]"
        else:
            user_query_for_backend = user_query
...
with col2:
    st.info("🗣️ **Languages**: Ask in English or Hindi, get responses in English or Hindi")
